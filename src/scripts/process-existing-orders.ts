...
async function processExistingOrders() {
  try {
    ...
    console.log(`📋 Found ${confirmedOrders.length} confirmed service orders without escrows`);
    ...
    for (const order of confirmedOrders) {
      try {
        ...
        console.log(`   Paystack Reference: ${order.paystackReference || 'N/A'}`);
        ...
        const result = await paymentsService.handlePaymentCallback(order.paystackReference);
