...
async function testEscrowFix() {
  try {
    ...
    console.log(`  Service Appointments: ${confirmedOrder.serviceAppointments.length}`);
    console.log(`  Paystack Reference: ${confirmedOrder.paystackReference || 'N/A'}`);
    ...
    const result = await paymentsService.handlePaymentCallback(confirmedOrder.paystackReference);
