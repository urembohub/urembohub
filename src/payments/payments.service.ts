...
export class PaymentsService {
  ...
  async handlePaymentCallback(reference: string): Promise<{
    ...
    try {
      ...
      await this.updateOrderStatusBasedOnPackages(order)
  ...
  private async handleManufacturerOrderPayment(reference: string, paymentData: any): Promise<{
    ...
    try {
      ...
      return {
        success: true,
        orderId: orderId,
        message: "Manufacturer order payment processed.",
      };
    } catch (error) {
   ...
   * Update order status based on package creation results
   */
  private async updateOrderStatusBasedOnPackages(order: any, packageResults: { success: boolean; failedPackages: any[]; totalPackages: number }={ success: false, failedPackages: [], totalPackages: 0 }) {
    try {
